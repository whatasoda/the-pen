import React, { useEffect, useState } from 'react';
import useFiberSounds from '../../fiber/useFiberSouds';
import useGlobalMotion from '../../shared/global-motion';

const freqs: [number, number][][] = [
  // 27.5,
  // 29.135,
  // 30.868,
  // 32.703,
  // 34.648,
  // 36.708,
  // 38.891,
  // 41.203,
  // 43.654,
  // 46.249,
  // 48.999,
  // 51.913,
  // 55.0,
  // 58.27,
  // 61.735,
  // 65.406,
  // 69.296,
  // 73.416,
  // 77.782,
  // 82.407,
  // 87.307,
  // 92.499,
  // 97.999,
  // 103.826,
  // 110.0,
  // 116.541,
  // 123.471,
  [[65.406, 1], [82.407, 1], [97.999, 1], [130.813, 1]],
  // 138.591,
  // 146.832,
  // 155.563,
  // 164.814,
  // 174.614,
  // 184.997,
  [[97.999, 1], [130.813, 1], [164.814, 1], [195.998, 1]],
  // 207.652,
  // 220.0,
  // 233.082,
  // 246.942,
  [[130.813, 1], [164.814, 1], [195.998, 1], [261.626, 1]],
  // 277.183,
  // 293.665,
  [[155.563, 1], [195.998, 1], [261.626, 1], [311.127, 1]],
  [[164.814, 1], [195.998, 1], [261.626, 1], [329.628, 1]],
  // 349.228,
  // 369.994,
  // 391.995,
  // 415.305,
  [[220.0, 1], [261.626, 1], [349.228, 1], [440.0, 1]],
  // 466.164,
  // 493.883,
  // [[523.251, 1]],
  // 554.365,
  // 587.33,
  // 622.254,
  // 659.255,
  // 698.456,
  // 739.989,
  // 783.991,
  // 830.609,
  // 880.0,
  // 932.328,
  // 987.767,
  // 1046.502,
  // 1108.731,
  // 1174.659,
  // 1244.508,
  // 1318.51,
  // 1396.913,
  // 1479.978,
  // 1567.982,
  // 1661.219,
  // 1760.0,
  // 1864.655,
  // 1975.533,
  // 2093.005,
  // 2217.461,
  // 2349.318,
  // 2489.016,
  // 2637.02,
  // 2793.826,
  // 2959.955,
  // 3135.963,
  // 3322.438,
  // 3520.0,
  // 3729.31,
  // 3951.066,
  // 4186.009
];

// const randFreqs = [...new Array(10).fill(0).map(() => Math.random() * 1000)];

const Byon = () => {
  useGlobalMotion.useModule();
  const fibers = useFiberSounds(freqs);

  const [, set] = useState(0);

  useEffect(() => {
    let i = 0;
    const update = () => {
      set(++i);
      requestAnimationFrame(update);
    };
    update();
  }, []);

  return (
    <div style={{ height: '100vh' }}>
      {fibers.map(({ id, state }) => (
        <div key={id}>
          {Object.entries(state).map(([key, value]) => (
            <>
              {key}: {value}
              <br />
            </>
          ))}
        </div>
      ))}
    </div>
  );
};

export default Byon;
